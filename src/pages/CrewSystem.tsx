...
import { Users, Heart, Brain, Star, TrendingUp, Clock, Award } from "lucide-react";
...
export default function CrewSystem() {
  ...
  const [activeTab, setActiveTab] = useState<"roster" | "skills" | "psychology" | "performance" | "schedule">("roster");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-space-800/50 p-1 rounded-lg border border-cyber-blue/10">
        {[
          { id: "roster", label: "船员花名册", icon: <Users size={12} /> },
          { id: "skills", label: "技能管理", icon: <Award size={12} /> },
          { id: "psychology", label: "心理健康", icon: <Brain size={12} /> },
          { id: "performance", label: "绩效追踪", icon: <TrendingUp size={12} /> },
          { id: "schedule", label: "排班管理", icon: <Clock size={12} /> },
        ].map((tab) => (
          <button
            key={tab.id}
